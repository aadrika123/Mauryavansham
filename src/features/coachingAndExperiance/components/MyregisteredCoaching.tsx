...
const MyRegisteredCoaching = () => {
  ...
  const [selectedCoaching, setSelectedCoaching] = useState<CoachingCenter | null>(null);
  const [loading, setLoading] = useState(true);

  const fetchCoachingCenters = async () => {
    try {
      if (!session?.user?.id) return;
      const res = await fetch(`/api/coaching-centers?userId=${session.user.id}`);
  ...
  return (
    <div className="p-6">
      <h1 className="text-2xl font-semibold mb-6">My Registered Coaching Centers</h1>
        ...
        <div className="fixed inset-0 bg-black/50 flex justify-center items-center z-50">
          <div className="bg-white rounded-xl shadow-lg w-[90%] max-w-2xl p-6 relative max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-2 text-gray-700">
              <p><strong>Owner:</strong> {selectedCoaching.ownerName}</p>
              <p><strong>Email:</strong> {selectedCoaching.email}</p>
              <p><strong>Phone:</strong> {selectedCoaching.phone}</p>
              <p>
                <strong>Address:</strong>{" "}
                {selectedCoaching.address}, {selectedCoaching.city},{" "}
                {selectedCoaching.state} - {selectedCoaching.pincode}
              </p>
              <p><strong>Status:</strong> {selectedCoaching.status}</p>
              <p>
                <strong>Registered On:</strong>{" "}
                {new Date(selectedCoaching.createdAt || "").toLocaleDateString()}
              </p>
            </div>
            ...
            {selectedCoaching.courses?.length > 0 ? (
              ...
              <p className="mt-4 text-sm text-gray-500">No courses added yet.</p>
            )}
            ...
            {/* Uploaded Documents */}
           {selectedCoaching.docUrls && selectedCoaching.docUrls.length > 0 && (
              <div className="mt-4">
