...
export async function createProfile(prevState: CreateProfileState, formData: FormData): Promise<CreateProfileState> {
  try {
    ...
    const rawData = {
      ...
      brothersDetails: formEntries.brothersDetails ? JSON.parse(formEntries.brothersDetails as string) : [],
      sistersDetails: formEntries.sistersDetails ? JSON.parse(formEntries.sistersDetails as string) : []
    };
