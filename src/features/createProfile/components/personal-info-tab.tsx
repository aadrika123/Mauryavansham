...
const AboutMeSuggestions = ({
  ...
  const generateSuggestions = () => {
    ...
    setTimeout(() => {
      ...
      const relationKey = relation === 'other' ? 'myself' : relation;

      let templates: string[] = [];

      if (
        aboutMeTemplates[genderKey] &&
        aboutMeTemplates[genderKey][relationKey as keyof (typeof aboutMeTemplates)[typeof genderKey]]
      ) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-start flex-wrap gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            {profileRelation && (
              <p className="text-sm text-orange-600 font-medium mt-1">Profile For: {getRelationDisplayText()}</p>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="space-y-2">
          ...
          />
          {hasError('name') && <p className="text-xs text-red-500 mt-1">{getFieldError('name')}</p>}
          {/* {isEmpty(data.name) && (
        ...
        <div className="space-y-2">
          ...
          />
          {hasError('dob') && <p className="text-xs text-red-500 mt-1">{getFieldError('dob')}</p>}
          {/* {isEmpty(data.dob) && (
        ...
        <div className="space-y-2">
          ...
          {isEmpty(data.gender) && <p className="text-xs text-red-500 mt-1">Gender is required</p>}
        </div>
        ...
        <div className="space-y-2">
          ...
          {hasError('phoneNo') ? (
            ...
            data.phoneNo.length !== 10 && <p className="text-xs text-red-500 mt-1">Phone number must be 10 digits</p>
          )}
        ...
        <div className="space-y-2">
          ...
          {isEmpty(data.email) ? (
            ...
            !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(data.email) && (
              <p className="text-xs text-red-500 mt-1">Please enter a valid email address (e.g., example@mail.com)</p>
            )
          )}
        </div>
        ...
        <div className="space-y-2">
          ...
          <div className="flex gap-2">
            <Select
              ...
              onValueChange={feet => {
                const inches = data.height?.split("'")[1]?.replace('"', '') || '0';
          ...
          {(!data.height || !data.height.includes("'") || data.height === "'") && (
        ...
        <div className="space-y-2">
          ...
          <Select value={data.maritalStatus} onValueChange={value => onUpdate({ maritalStatus: value })}>
            <SelectTrigger
              className={hasError('maritalStatus') ? 'border-red-500 focus:border-red-500 focus:ring-red-500' : ''}
            >
          ...
          {hasError('maritalStatus') && <p className="text-xs text-red-500 mt-1">{getFieldError('maritalStatus')}</p>}
        </div>
        ...
        <div className="space-y-2">
          ...
          <LanguagesInput value={data.languagesKnown} onChange={val => onUpdate({ languagesKnown: val })} />
        </div>
...
function LanguagesInput({ value, onChange }: { value: string; onChange: (val: string) => void }) {
    ...
    <div className="space-y-2">
      ...
      <div className="flex flex-wrap gap-2 mt-2">
        {languages.map((lang, idx) => (
          ...
          >
            {lang}
            <X className="w-4 h-4 cursor-pointer hover:text-red-600" onClick={() => removeLanguage(lang)} />
          </span>
        ))}
      </div>
    </div>
