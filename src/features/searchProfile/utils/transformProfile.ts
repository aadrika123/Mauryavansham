...
export function transformDatabaseProfileToProfile(
  ...
  const location =
    dbProfile.workLocation || dbProfile.ancestralVillage || "Not specified";
...
export function transformDatabaseProfilesToProfiles(
  ...
  console.log(
    `Transformed ${dbProfiles.length} database profiles to ${transformed.length} UI profiles`
  );
  return transformed;
}
