...
export async function getUserDashboardData(userId: number) {
  try {
    ...
    const activeProfilesOnly = transformedProfiles.filter(p => p.isActive && !p.isDeleted);
    ...
    const profilesStats = {
      ...
      relationWise: allProfiles.reduce((acc: Record<string, number>, p) => {
    ...
    const blogStats = {
      ...
      removed: userBlogs.filter(b => b.status === 'removed').length
    };
