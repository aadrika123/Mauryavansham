  ...
  return (
    <ToastProvider>
      {toasts.map(function ({ id, title, description, ...props }) {
        ...
        return (
          <Toast key={id} {...props} className={cn('flex items-start gap-2 p-4 rounded-lg shadow-lg', variantColor)}>
            <div className="grid gap-1">
              {/* Variant ko title ke tarah dikhayenge */}
              {title && <ToastTitle className="capitalize font-semibold">{title}</ToastTitle>}
