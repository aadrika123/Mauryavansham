...
import { Eye, Edit, UserX, Crown, ShieldCheck, MapPin, GraduationCap, Briefcase, Circle } from 'lucide-react';
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-md rounded-2xl shadow-2xl">
        <DialogHeader>
          <DialogTitle className="text-xl font-semibold text-gray-800">Deactivate Profile</DialogTitle>
        </DialogHeader>
...
export default function DashboardProfileList(props: any) {
  ...
  return (
    <div className="space-y-6 w-full sm:w-[90%] lg:w-[80%] mx-auto mb-10">
      {enrichedProfiles?.map((profile: any) => (
        ...
        >
            ...
            <div className="flex-grow">
              <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start">
                <h3 className="text-xl sm:text-2xl font-bold text-gray-800 capitalize flex items-center gap-2">
                  {profile.name}
                  {profile.isVerified && <ShieldCheck className="w-5 h-5 text-green-500" />}
              ...
              <p className="flex items-center text-sm text-gray-700 mt-1">
                <Briefcase className="w-4 h-4 mr-1" /> {profile.occupation} @ {profile.company}
              </p>
              {profile.gotra && (
          ...
          <CardContent className="flex flex-wrap gap-3 px-6 pb-6">
            <Button
              variant="outline"
              onClick={() => router.push(`/dashboard/view-profile/${profile.id}`)}
            ...
            <Button
              variant="outline"
              onClick={() => router.push(`/dashboard/edit-profile/${profile.id}`)}
              className="rounded-lg flex-1 sm:flex-none"
            >
            ...
            >
              ...
              {deactivatingProfile === profile.id ? 'Deactivating...' : 'Deactivate'}
            </Button>
            <Button
              variant="outline"
              onClick={() => router.push(`/dashboard/profile-interests/${profile.id}`)}
      ...
      {enrichedProfiles.length === 0 && <p className="text-center text-gray-600">No profiles found.</p>}
