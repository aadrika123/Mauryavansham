...
export async function GET(req: NextRequest) {
  try {
    ...
    if (dateFrom)
      conditions.push(gte(discussions.createdAt, new Date(dateFrom)));
    ...
    const data = await db
      .select({
        ...
        likesCount: sql<number>`COUNT(DISTINCT ${discussionLikes.id})`.as(
          "likesCount"
        ),
        repliesCount: sql<number>`COUNT(DISTINCT ${discussionReplies.id})`.as(
          "repliesCount"
        ),
        replyLikesCount:
          sql<number>`COUNT(DISTINCT ${discussionReplyLikes.id})`.as(
            "replyLikesCount"
          ),
      })
      .from(discussions)
      ...
      .leftJoin(
        discussionReplyLikes,
        eq(discussionReplies.id, discussionReplyLikes.replyId)
      )
