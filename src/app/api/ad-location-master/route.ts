...
export async function POST(req: NextRequest) {
  try {
    ...
    const existing = await db.select().from(adPlacements).where(eq(adPlacements.id, id));

    if (existing.length > 0) {
      return NextResponse.json({ error: `Ads Serial No. ${id} already exists` }, { status: 400 });
    }

    const [newPlacement] = await db.insert(adPlacements).values({ id, pageName, sectionName, description }).returning();
