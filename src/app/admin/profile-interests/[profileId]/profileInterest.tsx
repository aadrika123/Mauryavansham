...
export default function ProfileInterestsPage({ user }: { user: string }) {
  ...
  useEffect(() => {
    ...
    const fetchInterests = async () => {
      ...
      try {
        const res = await fetch(`/api/profile-interest/${profileId}/interests`, {
          cache: 'no-store'
        });
  ...
  if (error) return <p className="text-center py-8 text-red-500">{error}. Check your API path and server logs.</p>;
  ...
  return (
    <div className="max-w-5xl mx-auto px-6 py-10">
      ...
      {interests.length === 0 ? (
        <p className="text-gray-600 text-center py-10">Nobody has expressed interest yet.</p>
      ) : (
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {interests.map(interest => {
            ...
            return (
              <Card key={interest.id} className="rounded-2xl shadow-sm hover:shadow-md transition">
                <CardContent className="p-5 flex flex-col items-center text-center">
                  ...
                  <div className="w-full mt-4 flex gap-2">
                    <Button
                      ...
                      onClick={() => {
                        if (user == 'user') {
                          router.push(`/admin/search-profile/${interestId}`);
                        } else {
                          router.push(`/admin/search-profile/${interestId}`);
                        }
                      }}
                  ...
                  <p className="text-xs text-gray-400 mt-3">
                    {interest.createdAt ? new Date(interest.createdAt).toLocaleString() : ''}
                  </p>
                </CardContent>
              </Card>
            );
