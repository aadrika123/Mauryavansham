...
const ProfileRelationPopup = ({
  ...
  const handleSubmit = () => {
    if (selectedRelation) {
      onSelect(selectedRelation, selectedRelation === 'other' ? customRelation : undefined);
      onClose();
    }
  };
  ...
  return (
    <Dialog open={isOpen}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle className="text-xl font-semibold text-orange-700">Create Profile For?</DialogTitle>
          <DialogDescription>Please select who you are creating this profile for</DialogDescription>
        </DialogHeader>

        <div className="space-y-4">
          <RadioGroup value={selectedRelation} onValueChange={setSelectedRelation} className="grid grid-cols-2 gap-3">
            {profileRelations.map(relation => {
              const Icon = relation.icon;
              return (
                <div key={relation.value} className="flex items-center space-x-2">
          ...
          <div className="flex gap-3 pt-4">
            <Button onClick={() => router.push('/admin/overview')} variant="outline" className="flex-1 bg-transparent">
              Cancel
            </Button>
            <Button
              onClick={handleSubmit}
              disabled={!selectedRelation || (selectedRelation === 'other' && !customRelation)}
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-md mx-auto">
        <DialogHeader>
          ...
          <DialogDescription>Please fill all the required fields for {getStepName(currentStep)}:</DialogDescription>
        </DialogHeader>

        <div className="space-y-3 max-h-60 overflow-y-auto">
          {errorMessages.map(({ field, message }, index) => (
            <div key={index} className="flex items-start gap-2 p-3 bg-red-50 rounded-md border border-red-200">
...
export default function CreateProfileForm({ profile, type = 'create' }: CreateProfilePageProps) {
  ...
  const updateProfileData = (
    section: keyof Omit<ProfileData, 'id' | 'userId' | 'profileRelation' | 'customRelation'>,
    data: Partial<ProfileData[keyof Omit<ProfileData, 'id' | 'userId' | 'profileRelation' | 'customRelation'>]>
  ) => {
    setProfileData(prev => ({
      ...prev,
      [section]: {
  ...
  const validateCurrentStep = (step: number): boolean => {
    ...
    switch (step) {
      case 1: // Personal Info
        ...
        if (!profileRelation) {
          errors.profileRelation = 'Please select who you are creating this profile for';
        }
        if (profileRelation === 'other' && !customRelation.trim()) {
          errors.customRelation = 'Please specify the relationship';
        }
        if (!profileData.personalInfo.profileImage1 && !selectedImages.image1 && !imagePreviews.preview1) {
  ...
  const handleImageSelect = (imageNumber: 1 | 2 | 3, event: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const handleImageRemove = (imageNumber: 1 | 2 | 3) => {
    ...
    // Update profile data
    const imageField = imageNumber === 1 ? 'profileImage1' : imageNumber === 2 ? 'profileImage2' : 'profileImage3';
    updateProfileData('personalInfo', { [imageField]: '' });

    // Clear file input
    const fileInput =
      imageNumber === 1 ? fileInputRef1.current : imageNumber === 2 ? fileInputRef2.current : fileInputRef3.current;
    if (fileInput) {
      fileInput.value = '';
    }
  };
  ...
  const ImageUploadSlot = ({
    ...
    const fileInputRef = imageNumber === 1 ? fileInputRef1 : imageNumber === 2 ? fileInputRef2 : fileInputRef3;
    ...
    return (
      <div className="flex flex-col items-center">
          ...
          {preview ? (
            ...
            <div
              className={`w-24 h-24 rounded-lg bg-gray-100 border-2 border-dashed ${
                hasError ? 'border-red-300 bg-red-50' : 'border-orange-200 hover:border-orange-400'
            ...
            >
              {isUploading ? (
                ...
                <Plus className={`w-6 h-6 ${hasError ? 'text-red-400' : 'text-gray-400'}`} />
              )}
            </div>
          )}
        </div>
        ...
        {/* Image Label */}
        <p className={`text-xs mt-1 text-center ${hasError ? 'text-red-500' : 'text-gray-500'}`}>
  ...
  const handleCompleteProfile = (e: React.FormEvent) => {
    ...
    const hasImage = profileData.personalInfo.profileImage1 || selectedImages.image1 || imagePreviews.preview1;

    if (!hasImage) {
      setValidationErrors({
        profileImage1: 'At least one profile photo is required before completing the profile'
      });
      setShowValidationPopup(true);
      return;
    }
    ...
    startTransition(async () => {
      try {
        ...
        Object.entries(profileData.familyDetails).forEach(([key, value]) => {
          // Force array/object fields to be at least empty array
          if (key === 'brothersDetails' || key === 'sistersDetails') {
            const finalValue = Array.isArray(value) ? value : [];
            formData.append(key, JSON.stringify(finalValue));
            console.log(`Added ${key}:`, JSON.stringify(finalValue));
          } else if (value) {
        ...
        if (type === 'edit' && profileData.id) {
          ...
          if (result.success) {
            ...
            setTimeout(() => {
              router.push(`/admin/view-profile/${result?.data?.id || profileData.id}`);
            }, 1500);
          } else {
            toast({
              title: 'Update Failed',
          ...
          if (createResult.success) {
            ...
            if (createResult?.data?.id) {
              setTimeout(() => {
                router.push(`/admin/view-profile/${createResult.data.id}`);
              }, 1500);
            }
          } else {
            toast({
  ...
  const isAnyImageUploading = isUploadingImages.image1 || isUploadingImages.image2 || isUploadingImages.image3;
          ...
          <div className="max-w-full mx-auto">
            ...
            {validationErrors.profileRelation && (
              <div className="mb-4 p-3 bg-red-50 border border-red-200 rounded-md">
                <p className="text-red-600 text-sm">{validationErrors.profileRelation}</p>
              </div>
            )}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="order-2 lg:order-1 lg:col-span-1">
                <Card className="p-4 mb-4">
                  <h3 className="text-lg font-semibold mb-4 text-center text-orange-800">Profile Photos</h3>
                  ...
                  {validationErrors.profileImage1 && (
                    <div className="mb-4 p-2 bg-red-50 border border-red-200 rounded-md">
                      <p className="text-red-600 text-sm text-center">{validationErrors.profileImage1}</p>
                    </div>
                  )}
                  ...
                  <div className="mt-6 text-center">
                    <h4 className="text-sm font-medium text-orange-700 mb-3">Photo Guidelines</h4>
                  ...
                  <div className="flex gap-2 w-full sm:w-auto">
                    <Button
                      ...
                      className={`w-full sm:w-auto ${
                        currentStep === 5 ? 'bg-green-600 hover:bg-green-700' : 'bg-orange-600 hover:bg-orange-700'
                      }`}
