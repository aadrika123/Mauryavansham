...
export default function HeritageListPage() {
  ...
  if (loading) {
    return <div className="p-6 text-center text-gray-500">Loading heritage data...</div>;
  }
              ...
              {heritageList.map((item, index) => (
                <tr key={item.id} className="hover:bg-gray-50">
                  ...
                  <td className="p-3 border-b">
                    {item.imageUrl ? (
                      <img src={item.imageUrl} alt={item.title} className="w-16 h-12 object-cover rounded-md" />
                    ) : (
                  ...
                  <td className="p-3 border-b text-sm text-gray-600">{item.createdByName || '—'}</td>
                  <td className="p-3 border-b text-right">
                    <button
                      onClick={() => router.push(`/admin/createHeritage/formPage?id=${item.id}`)}
